...
# Convert state tuple to feature vector
def state_to_features(state):
    x, y = state
    return np.array([x / NUM_ROWS, y / NUM_COLS], dtype=np.float32)
...
pygame.display.set_caption("Deep Q-learning Grid with Landmines")  # Set the window title
...
epsilon_decay = 0.915  # Exponential decay factor per episode
num_episodes = 1000  # Number of episodes for training
batch_size = 32  # Batch size for training
replay_buffer_size = 10000  # Size of experience replay buffer
min_replay_size = 100  # Minimum experiences before training starts
...
while running:
    for event in pygame.event.get():
        ...
        if not simulation_started:
            ...
            if start_button.handle_event(event):
                ...
                q_network = DQN(input_size=2, hidden_size=64, output_size=4)
                target_network = DQN(input_size=2, hidden_size=64, output_size=4)
    ...
    if not simulation_started:
        ...
        # Deep Q-learning algorithm with Pygame visualization
        for episode in range(num_episodes):
            ...
            while not done:
                ...
                with torch.no_grad():
                    q_values_all = q_network(torch.FloatTensor(state_features).unsqueeze(0)).squeeze().numpy()
                    # Get Q-values for all states for visualization
                    q_table_vis = np.zeros((NUM_ROWS, NUM_COLS, 4))
                    for i in range(NUM_ROWS):
                        for j in range(NUM_COLS):
                            state_vis = state_to_features((i, j))
                            q_vals = q_network(torch.FloatTensor(state_vis).unsqueeze(0)).squeeze().numpy()
                            q_table_vis[i, j] = q_vals
                ...
                min_q = np.min(q_table_vis)
                max_q = np.max(q_table_vis)
                ...
                for i in range(NUM_ROWS):
                    for j in range(NUM_COLS):
                        ...
                        max_q_cell = np.max(q_table_vis[i, j])
                        cell_color = get_color(max_q_cell, min_q, max_q)
                        pygame.draw.rect(screen, cell_color, rect)
                        
                        # Draw Q-values
                        q_values = q_table_vis[i, j]
